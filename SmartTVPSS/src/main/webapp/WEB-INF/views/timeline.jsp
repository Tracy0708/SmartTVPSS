...
.status-not_assigned {
	background-color: #fcd88b !important;
	color: #666;
}
...
<body>
	...
	<div class="main-container">
		<div class="mt-5">
			...
			<h2 class="mb-4">Schools' Timeline</h2>
			...
			<a
				href="${pageContext.request.contextPath}/program/admin/requestTimeline">
				<div class="inbox">
					...
					<tbody id="schoolTableBody">
						<c:forEach items="${schoolList}" var="school" varStatus="status">
							...
							<tr data-school-id="${school.id}">
								<td><span
									class="status-chip status-${fn:toLowerCase(school.status)}">
										${school.status} </span></td>
								<td>${status.count}</td>
								<td>${school.schoolCode}</td>
								<td>${school.schoolName}</td>
								<td>
									<button class="btn btn-sm btn-primary edit-timeline"
										data-school-id="${school.id}">
										<i class="bi bi-calendar3"></i>
									</button> <span class="timeline-text" id="timeline-display-${school.id}">
										<c:choose>
											<c:when
												test="${not empty school.timelineStart and not empty school.timelineEnd}">
